...
async function loadResults(id) {
  try {
    ...
    const category = result.performance_category || "Completed";
    catTag.textContent = category;

    if (score >= 85) catTag.style.color = "var(--success)";
    else if (score >= 70) catTag.style.color = "var(--primary)";
    else if (score >= 50) catTag.style.color = "var(--warning)";
    else catTag.style.color = "var(--danger)";
    ...
    document.getElementById("text-strengths").textContent = result.strengths || "Good fundamental effort.";
    document.getElementById("text-weakness").textContent = result.weak_areas || "Technical depth and syntax details.";
    document.getElementById("text-recommendations").textContent = result.recommendations || "Practice further interview questions.";
        ...
        <div class="review-card">
          <div class="review-header" onclick="toggleAccordion(this)">
            <div style="display:flex; align-items:center;">
              <span class="badge badge-teal" style="margin-right:0.6rem;">Q${r.question_order}</span>
              <strong style="color:var(--text-primary); font-size:0.95rem;">${r.question_text}</strong>
            </div>
            <div style="display:flex; align-items:center; gap:0.5rem;">
              <span class="badge badge-teal">${qScore} / 100</span>
              <span style="font-size:0.8rem; color:var(--text-muted);">▼</span>
            </div>
          </div>
          <div class="review-body collapsed">
            <div style="margin-bottom:1rem;">
              <strong style="color:var(--text-secondary); font-size:0.85rem;">Your Answer:</strong>
              <div style="background:var(--surface); padding:0.85rem; border-radius:var(--radius-sm); border:1px solid var(--border); margin-top:0.3rem; font-size:0.9rem;">
                ${r.user_answer ? r.user_answer : "<em>No answer provided.</em>"}
              </div>
            </div>

            <div class="eval-box eval-box-well">
              <strong>Feedback:</strong> ${r.feedback || "Good attempt."}
            </div>

            <div class="eval-box eval-box-improve">
              <strong>How to Improve:</strong> ${r.improvement_suggestion || "Expand technical explanation."}
            </div>

            <div class="eval-box eval-box-ideal">
              <strong>Ideal Answer:</strong> ${r.ideal_answer || "N/A"}
            </div>
          </div>
        </div>
      `;
